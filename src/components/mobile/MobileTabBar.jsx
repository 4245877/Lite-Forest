...
export default function MobileTabBar() {
  ...
  // Поточний користувач: визначає ціль вкладки «Аккаунт»
  const { user } = useAuth();
  ...
  return (
    <nav className="mobile-tabbar" role="navigation" aria-label="Нижня навігація">
      {/* 1. Головна */}
      ...
      <NavLink to="/cart" className="mtb-link mtb-cart">
        {({ isActive }) => (
          <>
            ...
            {cartCount > 0 && (
              <span className="mtb-badge" aria-label={`У кошику ${cartCount}`}>
      ...
      {/* 5. Аккаунт */}
      <NavLink to={accountTo} className="mtb-link">
        {({ isActive }) => (
          <>
