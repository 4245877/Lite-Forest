...
function formatPrice(value) {
  ...
  if (!Number.isFinite(price)) return 'Ціну уточнюйте';
...
const ProductCard = ({
  ...
  // When a parent supplies the favorites set, this card derives its state from
  // it instead of issuing its own /favorites request (avoids N+1 in listings).
  const favoritesControlled = favoriteIds instanceof Set;
  ...
  useEffect(() => {
    ...
    if (favoritesControlled) {
      setIsFavorite(favoriteIds.has(String(productId)));
      return undefined;
    }
  ...
  }, [user, productId, favoritesControlled, favoriteIds]);
      ...
      <div className="product-info">
        ...
        {availabilityLabel ? (
          <div className="product-availability-row">
            <span className={`product-availability ${availabilityState || ''}`}>
              {availabilityLabel}
            </span>
            {availabilityTiming ? (
              <span className="product-availability-timing">{availabilityTiming}</span>
            ) : null}
          </div>
        ) : null}
