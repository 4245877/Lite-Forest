...
const Footer = () => {
  ...
  const [newsletterState, setNewsletterState] = useState({ status: 'idle', msg: '' });
  ...
  const submitNewsletter = async (e) => {
    ...
    setNewsletterState({ status: 'sending', msg: '' });
    try {
      ...
      if (!res.ok) {
        setNewsletterState({ status: 'error', msg: body?.message || 'Не вдалося оформити підписку.' });
        return;
      }
      setNewsletterEmail('');
      setNewsletterState({ status: 'ok', msg: 'Дякуємо! Ви підписані.' });
    } catch {
      setNewsletterState({ status: 'error', msg: 'Не вдалося оформити підписку. Спробуйте пізніше.' });
    }
  };
            ...
            <div className="newsletter-signup">
              ...
              <form className="newsletter-form" onSubmit={submitNewsletter}>
                ...
                <button
                  ...
                  disabled={newsletterState.status === 'sending'}
                >
                  →
                </button>
              </form>
              {newsletterState.msg ? (
                <p
                  ...
                  role="status"
                  style={{ color: newsletterState.status === 'error' ? '#e57373' : 'inherit' }}
                >
                  {newsletterState.msg}
                </p>
              ) : null}
