...
// P0-6: админ-секрет НЕ хранится в браузерном бандле. Раньше VITE_ADMIN_TOKEN
// вшивался в публичный bundle, и любой посетитель мог извлечь его и получить
// admin-доступ. Теперь админ-действия авторизуются httpOnly JWT-кукой (роль
// 'admin' в БД) — запросы и так идут с credentials:'include' (см. fetchJson).
...
async function fetchJson(url, init = {}, { allowNoContent = false } = {}) {
  ...
  if (!response.ok) {
    ...
    const error = new Error(`HTTP ${response.status} ${response.statusText} for ${url}: ${body}`);

    // Текст сообщения оставляем как был (на него смотрят логи и существующий код),
    // но машиночитаемую часть ответа кладём рядом: разбирать код отказа из строки
    // вызывающему коду нечестно и хрупко. Тело не всегда JSON — тогда просто нет.
...
export const api = {
  // Котировка заказа: сервер считает доставку, промокод, итог и сумму к оплате
  // и отдаёт ровно те цифры, которые запишет оформление. Витрина их только
  // показывает — свой параллельный расчёт неизбежно разъезжается с серверным.
  async quoteOrder(payload, opts = {}) {
    const url = makeUrl('/orders/quote');
  ...
  // Наличие филаментов на складе. Бекенд магазина проксирує запит до фулфілменту
  // (з bearer-токеном, кешем і stale-on-error) — браузер на 192.168.0.139
  // напряму НЕ ходить і токен не бачить. Відповідь: стабільний внутрішній
  // контракт { ok, degraded, stale, source_updated_at, threshold_g, items }.
  async getFilamentAvailability(opts = {}) {
    const url = makeUrl('/filament-availability');
    const extraHeaders = headersToObject(opts.headers);
  ...
  // Мини-чат «Питання майстру» зі сторінки товару. Повідомлення йде на бекенд
  // (/api/product-questions), а той пересилає його у зовнішній сервіс обращень
  // (192.168.0.139) — адреса сервісу живе лише в env бекенда, фронт її не знає
  // (той самий патерн, що й синхронізація замовлень із фулфілментом).
  //
  // TODO(backend): коли сервіс «Звернення» буде підключено, достатньо виставити
  // на бекенді PRODUCT_QUESTIONS_URL — фронт міняти не потрібно.
  async sendProductQuestion(payload, opts = {}) {
    const url = makeUrl('/product-questions');
    const extraHeaders = headersToObject(opts.headers);
  ...
  // Обратная сторона мини-чата: подтягиваем ответы оператора по thread_id.
  // Бекенд проксирует запрос в сервіс «Звернення» (GET-сторона ingest) и
  // возвращает { ok, thread_id, status, messages:[{id,author,text,created_at}] }.
  //
  // threadToken — підпис треда з відповіді POST: чат гостьовий, тож доступ до
  // переписки дає саме він, а не thread_id. Шлемо заголовком, щоб токен не
  // потрапляв у логи nginx і Referer.
  async getProductQuestionThread(threadId, threadToken, opts = {}) {
    const url = makeUrl(`/product-questions/${encodeURIComponent(threadId)}`);
    const extraHeaders = headersToObject(opts.headers);
  ...
  // Покупка заявки: бэкенд создаёт заказ, кладёт его в профиль и шлёт файл+
  // параметры в фулфилмент. Идемпотентно — повторный вызов вернёт тот же заказ.
  // payload — необязательные данные формы оформления (контакт/адрес/оплата);
  // без него бэкенд создаёт заказ с дефолтами (обратная совместимость).
  async purchaseCustomPrintRequest(id, payload = null, opts = {}) {
    const url = makeUrl(`/custom-print/requests/${encodeURIComponent(id)}/purchase`);
    const extraHeaders = headersToObject(opts.headers);
