// src/pages/LegalPages.jsx — сторінка «Юридична інформація» (редизайн)
...
const SITE_HOST = WEBSITE_URL.replace(/^https?:\/\//, "").replace(/\/$/, ""); // домен без схеми, для прози
...
const THIRD_PARTIES = {
  ...
  hosting: ["хмарна інфраструктура та CDN"],
};

// Розділи сторінки (порядок = порядок секцій у контенті)
const SECTIONS = [
  { id: "safe-deal", label: "Безпечна угода", icon: "🛡️" },
  { id: "shipping-returns", label: "Доставка та повернення", icon: "📦" },
  { id: "privacy", label: "Конфіденційність", icon: "🔒" },
  { id: "terms", label: "Угода користувача", icon: "📄" },
...
export default function LegalPages() {
  ...
  // Навігація по хешу (перехід за посиланням, оновлення URL)
  useEffect(() => {
  ...
  // Scroll-spy + кнопка «догори» (один слухач, rAF-throttle, стабільний)
  useEffect(() => {
    let ticking = false;
    ...
    const compute = () => {
      ...
      const line = 140; // лінія визначення активної секції під шапкою
      let current = SECTIONS[0].id;
      for (const s of SECTIONS) {
        const el = document.getElementById(s.id);
        if (el && el.getBoundingClientRect().top - line <= 0) current = s.id;
      }
      // біля кінця сторінки підсвічуємо останній розділ
  ...
  // Мобільна стрічка пілюль: підтягуємо активний розділ у видиму зону (стрічка гориз. скролиться)
  useEffect(() => {
    const nav = navRef.current;
    if (!nav || nav.scrollWidth <= nav.clientWidth) return;
    const pill = nav.querySelector('[data-active="true"]');
    if (!pill) return;
    ...
    <div className="legal">
      <header className="legal__hero">
        <div className="legal__hero-inner">
          <span className="legal__eyebrow">{COMPANY_NAME}</span>
          <h1 className="legal__title">Юридична інформація</h1>
          <p className="legal__subtitle">
            Прозорі умови покупки, захист персональних даних і зрозумілі правила — усе, що варто знати перед
            замовленням.
          </p>
          <p className="legal__updated">
            <span className="legal__updated-dot" aria-hidden="true" />
      ...
      <div className="legal__layout">
        <aside className="legal__aside" aria-label="Швидка навігація по розділах">
          <nav className="legal__nav" ref={navRef}>
            <p className="legal__nav-title">Розділи</p>
            {SECTIONS.map((s) => (
              <Link
                key={s.id}
                to={`/legal#${s.id}`}
              ...
              >
                <span className="legal__nav-icon" aria-hidden="true">
                  {s.icon}
                </span>
                <span className="legal__nav-label">{s.label}</span>
              </Link>
            ))}
          </nav>
        </aside>
        ...
        <div className="legal__content">
          {/* ===== Безпечна угода ===== */}
          <section id="safe-deal" className="legal-card" aria-labelledby="safe-deal-title">
            <h2 id="safe-deal-title">Безпечна угода</h2>
            <p>
              Під «безпечна угода» ми маємо на увазі прозорі та захищені умови покупки на <SiteLink /> у{" "}
              {COMPANY_NAME}
              {legalEntityClause(LEGAL_ENTITY)}. Ми співпрацюємо з надійними платіжними провайдерами (
              {joinOrDash(THIRD_PARTIES.payments)}) та службами доставки ({joinOrDash(THIRD_PARTIES.shipping)}),
              дотримуємося вимог щодо захисту персональних даних і надаємо зрозумілий порядок повернень.
            </p>

            <h3>Як це працює</h3>
            <ul>
              <li>
                <strong>Оплата</strong>: транзакції обробляються через {joinOrDash(THIRD_PARTIES.payments)}. Ми не
                зберігаємо повні реквізити банківських карток.
              </li>
              <li>
                <strong>Підтвердження замовлення</strong>: після оформлення ви отримуєте підтвердження на email/в
                акаунті.
              </li>
              <li>
                <strong>Доставка</strong>: виконується через {joinOrDash(THIRD_PARTIES.shipping)}. Строки й тарифи
                відображаються під час оформлення.
              </li>
              <li>
                <strong>Повернення</strong>: умови та строки описані у розділі{" "}
                <Link to="/legal#shipping-returns">«Доставка та повернення»</Link>. Ми діємо згідно з чинним
            ...
            <ul>
              ...
              <li>
                ...
                <Link to="/legal#privacy">Політики конфіденційності</Link>.
              </li>
              <li>
            ...
            <ul>
              ...
              <li>
                Email: <EmailLink />
                {CONTACT_PHONE ? <>, Телефон: <PhoneLink /></> : null}
              </li>
            </ul>

            <p className="legal-note">
              Примітка: окремі деталі проведення платежів (блокування/списання коштів, 3-D Secure, повернення)
              регулюються політиками відповідного платіжного провайдера.
            </p>
          </section>

          {/* ===== Умови доставки та повернення ===== */}
          <section id="shipping-returns" className="legal-card" aria-labelledby="shipping-returns-title">
            <h2 id="shipping-returns-title">Умови доставки та повернення</h2>
            <p>
              Цей розділ описує, як ми доставляємо замовлення на <SiteLink /> і як працює повернення/обмін товарів.
              Доставку здійснюють партнери {joinOrDash(THIRD_PARTIES.shipping)}. Актуальні варіанти, строки та тарифи
              відображаються під час оформлення замовлення.
            </p>
            ...
            <ul>
              <li>
                Ви можете повернути товар протягом 14 календарних днів із дати отримання, відповідно до законодавства
            ...
            <ol>
              <li>
                Напишіть нам: <EmailLink />
                {CONTACT_PHONE ? <> або телефонуйте: <PhoneLink /></> : null}.
              </li>
              <li>
                Заповніть коротку форму повернення (за наявності), вкажіть номер замовлення, причину та фото (за
                потреби).
              </li>
            ...
            <p>
              Непродовольчі товари індивідуального виготовлення/замовлення, розпаковані витратні матеріали, товари
              особистої гігієни тощо можуть не підлягати поверненню, якщо це передбачено законом. Конкретний перелік
              залежить від категорії товару та вимог чинного законодавства.
            </p>
            ...
            <p className="legal-note">
              Деталі можуть відрізнятися залежно від категорії товару та політик перевізника. Завжди перевіряйте
              інформацію при оформленні замовлення та в цьому розділі.
            </p>
          </section>

          {/* ===== Політика конфіденційності ===== */}
          <section id="privacy" className="legal-card" aria-labelledby="privacy-title">
            <h2 id="privacy-title">Політика конфіденційності</h2>
            <p>
              Ця Політика пояснює, як {COMPANY_NAME}
              {legalEntityClause(LEGAL_ENTITY)} (далі — «ми», «нас», «наш») збирає, використовує, зберігає та захищає
              персональні дані користувачів під час відвідування сайту <SiteLink /> та користування нашими сервісами.
              Ми діємо відповідно до чинного законодавства України щодо захисту персональних даних та загальновизнаних
            ...
            <ul>
              ...
              <li>
                <strong>Платіжні дані</strong>: токени/ідентифікатори платежів (обробка здійснюється платіжними
                провайдерами: {joinOrDash(THIRD_PARTIES.payments)}). Ми не зберігаємо повні реквізити карток.
              </li>
              <li>
                <strong>Дані доставки</strong>: ПІБ одержувача, адреса/відділення, контакти (через служби доставки:{" "}
                {joinOrDash(THIRD_PARTIES.shipping)}).
              </li>
              <li><strong>Комунікації</strong>: звернення до підтримки, відгуки, участь в опитуваннях, підписка на розсилку.</li>
              <li>
                <strong>Технічні дані</strong>: IP-адреса, файли cookie, ідентифікатори пристрою/браузера, лог-файли,
                події взаємодії (аналітика: {joinOrDash(THIRD_PARTIES.analytics)}).
              </li>
            </ul>
            ...
            <ul>
              ...
              <li>
                Аналітичні/маркетингові платформи:{" "}
                {joinOrDash(THIRD_PARTIES.analytics.concat(THIRD_PARTIES.marketing || []))}.
              </li>
            ...
            <h3>7. Ваші права</h3>
            <p>
              Ви маєте права: доступу, виправлення, видалення, обмеження обробки, заперечення, переносимості даних, а
              також право відкликати згоду. Щоб реалізувати права, звертайтесь: <EmailLink />. Також ви можете
              звернутися зі скаргою до уповноважених органів.
            </p>

            <h3>8. Cookies</h3>
            <p>
              Ми використовуємо файли cookie та подібні технології. Детальніше — у{" "}
              <Link to="/legal#cookies">Політиці щодо файлів cookie</Link>.
            </p>
          ...
          {/* ===== Угода користувача ===== */}
          <section id="terms" className="legal-card" aria-labelledby="terms-title">
            <h2 id="terms-title">Угода користувача</h2>
            <p>
              Ця Угода регулює умови доступу та використання сайту <SiteLink /> і сервісів {COMPANY_NAME}.
              Користуючись сайтом, ви підтверджуєте, що прочитали та приймаєте цю Угоду і пов’язані політики.
            </p>
            ...
            <p>
              Використання сайту означає згоду з Угодою,{" "}
              <Link to="/legal#privacy">Політикою конфіденційності</Link> та{" "}
              <Link to="/legal#cookies">Політикою щодо файлів cookie</Link>. Якщо ви не згодні — припиніть користування
              сайтом.
            </p>
            ...
            <ul>
              ...
              <li>
                Оплата здійснюється через платіжні сервіси: {joinOrDash(THIRD_PARTIES.payments)}. Безпека платежів
                забезпечується їхніми політиками.
              </li>
            ...
            <ul>
              <li>
                Доставка здійснюється службами: {joinOrDash(THIRD_PARTIES.shipping)}. Строки та тарифи — згідно з
                інформацією під час оформлення.
              </li>
            ...
            <p>
              Умови повернення/обміну та гарантійного обслуговування визначені у розділі{" "}
              <Link to="/legal#shipping-returns">«Доставка та повернення»</Link> та застосовному законодавстві.
            </p>
            ...
            <ul>
              ...
              <li>
                Заборонені: спроби несанкціонованого доступу, втручання у роботу сайту, шахрайство, публікація
                незаконного/образливого контенту, порушення прав ІВ.
              </li>
            </ul>
            ...
            <ul>
              <li>
                Контент сайту (дизайн, тексти, зображення, логотипи, код) належить {COMPANY_NAME} або використовується
                на законних підставах.
              </li>
              <li>Ви отримуєте обмежену невиключну ліцензію користування сайтом для особистих некомерційних цілей.</li>
              <li>
                Розміщуючи відгуки/матеріали, ви надаєте нам невиключну ліцензію на їх публікацію та використання з
                посиланням на вас (за потреби).
              </li>
            </ul>
            ...
            <ul>
              ...
              <li>
                Відповідальність {COMPANY_NAME} обмежується межами, дозволеними законодавством, і не включає непрямі
                збитки, втрату прибутку тощо.
              </li>
            </ul>
            ...
            <p>
              Застосовується право України. Спори вирішуються шляхом переговорів, а у разі недосягнення згоди — у
              судовому порядку.
            </p>
          ...
          <section id="cookies" className="legal-card" aria-labelledby="cookies-title">
            <h2 id="cookies-title">Політика щодо файлів cookie</h2>
            <p>Ця Політика пояснює, які cookie та подібні технології ми використовуємо на <SiteLink />, з якою метою і як ви можете керувати налаштуваннями.</p>
            ...
            <ul>
              ...
              <li>
                <strong>Аналітичні</strong> — допомагають розуміти використання сайту й покращувати його (напр.,{" "}
                {joinOrDash(THIRD_PARTIES.analytics)}).
              </li>
              <li>
                <strong>Маркетингові</strong> — показ персоналізованої реклами та вимірювання її ефективності (напр.,{" "}
                {joinOrDash(THIRD_PARTIES.marketing)}).
              </li>
            </ul>
            ...
            <div className="legal-table-wrap">
              <table className="legal-table">
                ...
                <tbody>
                  <tr>
                    <td>liteforest_session</td>
                    <td>{COMPANY_NAME}</td>
                    <td>Сесія користувача (автентифікація, кошик, оформлення).</td>
                    <td>Сесійний</td>
                    <td>Строго необхідний</td>
                  </tr>
                  <tr>
                    <td>_ga, _gid</td>
                    <td>Google Analytics</td>
                    <td>Аналітика відвідуваності та подій.</td>
                    <td>2 роки / 24 год</td>
                    <td>Аналітичні</td>
                  </tr>
                  <tr>
                    <td>_fbp</td>
                    <td>Meta (Facebook)</td>
                    <td>Оптимізація рекламних кампаній.</td>
                    <td>3 місяці</td>
                    <td>Маркетингові</td>
                  </tr>
                </tbody>
            ...
            <p className="legal-note">
              Точний перелік cookie може змінюватися залежно від увімкнених сервісів і ваших налаштувань згоди.
            </p>
            ...
            <p>
              ...
              {joinOrDash((THIRD_PARTIES.analytics || []).concat(THIRD_PARTIES.marketing || []))}), регулюються
              політиками відповідних провайдерів.
            </p>
...
function legalEntityClause(entity) {
  return entity ? `, що діє як володілець даних (${entity})` : "";
}

// Клікабельні контакти (узгоджено з ContactPage)
function EmailLink() {
  return <a href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a>;
}

function PhoneLink() {
  return <a href={`tel:${CONTACT_PHONE.replace(/[^\d+]/g, "")}`}>{CONTACT_PHONE}</a>;
}
