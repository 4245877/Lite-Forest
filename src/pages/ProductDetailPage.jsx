...
function ProductDetailPageInner() {
  ...
  const stock = toSafeNumber(activeVariant?.stock ?? getProductValue(product, 'stock'), 0);
  const leadDays = toSafeNumber(activeVariant?.lead_time_days ?? getProductValue(product, 'lead_time_days'), 0);
  const explicitAvailability = String(getProductValue(product, 'availability') ?? '').trim().toLowerCase();

  const availability = (() => {
    if (
      stock > 0 ||
      explicitAvailability === 'in-stock' ||
      explicitAvailability === 'in_stock' ||
  ...
  function buildCartProduct() {
    ...
    return {
      ...
      cartLineId: `${productId}:${variantId ?? (optionsKey || 'default')}`,
      optionsKey,
  ...
  function addToCart() {
    ...
    addItem(buildCartProduct(), qty);

    alert(
      `Додано до кошика: ${productName} × ${qty}${
        selectionSummary ? `\n${selectionSummary}` : ''
      }`
    );
  }
  ...
  return (
    <div className="product-detail container">
      ...
      <section className="pd-contact">
        <button type="button" className="btn-primary" onClick={() => navigate('/contact?topic=question')}>
