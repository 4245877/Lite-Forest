...
// Ровно то, что отменяет API (CANCELABLE_STATUSES в backend/src/api/orders.ts).
// Списки разъехались: здесь стоял 'processing' (API на него отвечает 409, кнопка
// показывалась и не работала) и не было 'pending_payment' — статуса, с которым
// заказ создаётся, то есть кнопки не было именно там, где отмена разрешена.
const CANCELLABLE_STATUSES = new Set(['new', 'pending', 'pending_payment']);

// Ключ идемпотентности повтора — как в CartContext.checkout (makeId там же).
...
const getPaymentStatusClass = (status) => {
  if (status === 'paid_full') return styles.statusDelivered;
  if (status === 'paid_deposit') return styles.statusShipped;
...
// Ссылки ведём ТОЛЬКО туда, где есть маршрут или внешний url. Раньше здесь
// строились /orders/:id, /orders/:id/payment, /orders/:id/tracking,
// /orders/:id/review и /support/orders/:id — ни одного такого маршрута в
// AppRouter нет, и все они уходили в wildcard, то есть кнопки молча выкидывали
// пользователя на главную. Пока страниц нет, кнопки не рисуем.

// Оплата: только ссылка от сервера. Своего платёжного маршрута у витрины нет
// (платёж-переход не подключён), поэтому без paymentUrl кнопки быть не должно.
const getPaymentUrl = (order) =>
  order?.paymentUrl || order?.payment?.url || order?.invoiceUrl || '';

// /support существует и редиректит на /contacts; /support/orders/:id — нет.
const getSupportUrl = () => '/support';

// Только внешний трекинг (Нова Пошта и т.п.). Своей страницы отслеживания нет,
// а ТТН и так показан текстом ниже.
const getTrackingHref = (order) => {
  const trackingUrl = getTrackingUrl(order);
  return isExternalUrl(trackingUrl) ? trackingUrl : '';
};
...
const OrdersTab = () => {
  ...
  // Ключ на попытку повтора, по одному на исходный заказ. Живёт до успеха:
  // повторный клик после сетевой ошибки шлёт ТОТ ЖЕ ключ, поэтому сервер вернёт
  // уже созданный заказ вместо дубля. После успеха ключ сбрасывается — осознанный
  // второй повтор того же заказа обязан создать второй заказ.
  const repeatKeysRef = useRef({});
  ...
  const repeatOrder = async (id) => {
    ...
    if (!repeatKeysRef.current[orderId]) {
      repeatKeysRef.current[orderId] = makeIdempotencyKey();
    }

    try {
      const res = await fetch(`/api/orders/${encode(orderId)}/repeat`, {
        method: 'POST',
        headers: { 'Idempotency-Key': repeatKeysRef.current[orderId] },
        credentials: 'include',
      });

      if (!res.ok) throw new Error(await readError(res));

      // Заказ создан (или уже был создан этим же ключом) — следующий повтор
      // получит новый ключ и создаст новый заказ.
      delete repeatKeysRef.current[orderId];
    } catch (e) {
  ...
  return (
    <section className={cx(panel.section, styles.orders)} aria-labelledby="orders-tab-heading">
      <div className={styles.header}>
        <div>
          <h2 id="orders-tab-heading" className={styles.title}>
            Замовлення
          </h2>

          <p className={styles.subtitle}>
            Тут можна відстежити, повторити, скасувати або вирішити питання щодо
            замовлення.
          </p>
        </div>
      </div>
        ...
        <>
          <ul className={styles.ordersList}>
            {items.map((order, index) => {
              ...
              const trackingHref = getTrackingHref(order);
              const paymentHref = getPaymentUrl(order);
              const supportHref = getSupportUrl();

              const canTrack = Boolean(trackingHref);
              const canPay = status === PAYABLE_STATUS && Boolean(paymentHref);
                ...
                <li key={orderId || index} className={styles.orderItem}>
                  ...
                  <div className={styles.orderFoot}>
                    ...
                    {order.payment_status && (
                      <span className={cx(styles.status, getPaymentStatusClass(paymentStatus))}>
