...
import {
  ...
  customPrintStorageKeys,
  getDefaultMaterialForPrintType,
  getMaterialOptions,
  purgeLegacyCustomPrintStorage,
...
// Лише чернетки цього браузера ('local-…'): решту історії дає сервер, і саме він
// вирішує, що належить поточному користувачу.
function getLocalFallbackHistory(storageKeys) {
  return safeReadJson(storageKeys.history, []).filter((item) =>
...
export default function CustomPrintTab() {
  ...
  const { user } = useAuth();
  // Ключі сховища залежать від акаунта: після зміни користувача читаємо/пишемо
  // вже інший простір, тож чужа історія не «перетікає» в новий сеанс.
  const storageKeys = useMemo(() => customPrintStorageKeys(user?.id), [user?.id]);
  ...
  const persistHistory = useCallback(
    (nextItems) => {
      ...
      setHistory(sorted);
      safeWriteJson(storageKeys.history, sorted);
    },
    [storageKeys],
  );
  ...
  const activateItem = useCallback((item, file = null) => {
    ...
    setActiveId(item.id);
    safeWriteString(storageKeys.activeId, item.id);
    setValues({
      ...createEmptyCustomPrintDraft(),
      ...(item.values || {}),
      id: item.id,
  ...
  function resetForm() {
    ...
    setErrors({});
    setActiveId('');
    safeWriteString(storageKeys.activeId, '');
  }
  ...
  const refreshRemoteHistory = useCallback(async () => {
    ...
    const merged = mergeHistoryLists(getLocalFallbackHistory(storageKeys), mapped);

    persistHistory(merged);

    const currentId = safeReadString(storageKeys.activeId) || activeId;
    const current = merged.find((item) => item.id === currentId);

    if (current) {
      activateItem(current, null);
    }
  }, [activeId, activateItem, persistHistory, storageKeys]);

  // Прибираємо спільні v1-ключі: там лежить історія попереднього користувача.
  useEffect(() => {
    purgeLegacyCustomPrintStorage();
  }, []);

  // Локальний стан читаємо заново на кожну зміну акаунта, інакше в новому
  // сеансі на екрані лишалася б історія попереднього користувача.
  useEffect(() => {
    const localHistory = sortHistory(safeReadJson(storageKeys.history, []));
    setHistory(localHistory);

    const savedId = safeReadString(storageKeys.activeId);
    const initialItem =
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ПОРОЖНЯ серверна історія — теж відповідь: означає, що в цього
        // користувача заявок немає. Раніше ми на ній виходили достроково
        // (`!mapped.length` → return), тож на екрані лишалися чужі/застарілі
        // записи. Тепер завжди зводимо стан із сервером, лишаючи тільки
        // локальні чернетки цього браузера.
        const merged = mergeHistoryLists(getLocalFallbackHistory(storageKeys), mapped);
        persistHistory(merged);

        const storedId = safeReadString(storageKeys.activeId);
        if (!merged.some((item) => item.id === storedId)) {
          if (merged[0]) activateItem(merged[0], null);
          else resetForm();
        }
      } catch {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activateItem, persistHistory, storageKeys]);
