...
export default function CustomPrintOptionsForm({
  ...
  const handlePrintTypeChange = (nextPrintType) => {
    const nextMaterialOptions = getMaterialOptions(nextPrintType);
    const hasCurrentMaterial = nextMaterialOptions.some(
      (option) => option.value === formValues.material,
    );

    onChange?.('printType', nextPrintType);

    if (!hasCurrentMaterial) {
      onChange?.('material', nextMaterialOptions[0]?.value ?? '');
    }
  };
  ...
  return (
    <section className={panel.card}>
      <h3 className={panel.cardTitle}>3. Параметри друку</h3>
