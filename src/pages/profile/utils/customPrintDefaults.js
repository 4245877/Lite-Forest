...
// Історія custom-print у localStorage прив'язана до КОРИСТУВАЧА.
//
// Раніше ключі були спільними (…history.v1) — тобто на спільному комп'ютері
// після зміни акаунта нова людина бачила в історії чужі моделі, імена файлів і
// посилання на них. Тепер у ключ входить id користувача, тож дані різних
// акаунтів фізично не перетинаються, а гість має власний ('anon') простір.
const STORAGE_PREFIX = 'lite-forest.custom-print';

// v1-ключі лишилися від спільного сховища й містять чужу історію — прибираємо.
const LEGACY_STORAGE_KEYS = [
  `${STORAGE_PREFIX}.history.v1`,
  `${STORAGE_PREFIX}.active-id.v1`,
];

export function customPrintStorageKeys(userId) {
  const scope = String(userId || '').trim() || 'anon';

  return {
    history: `${STORAGE_PREFIX}.history.v2.${scope}`,
    activeId: `${STORAGE_PREFIX}.active-id.v2.${scope}`,
  };
}

export function purgeLegacyCustomPrintStorage() {
  if (typeof window === 'undefined') return;

  try {
    LEGACY_STORAGE_KEYS.forEach((key) => window.localStorage.removeItem(key));
  } catch {
    // localStorage may be unavailable in private browsing or strict modes.
  }
}
