...
//   'made-to-order' — явный индивидуальный/предзаказный товар; в витрине без
//                     текста (см. STATUS_LABELS), но с PreOrder в JSON-LD
...
// 'made-to-order' намеренно без текста: весь ассортимент изготавливается
// индивидуально, поэтому бейдж у каждого товара — шум. Об индивидуальном
// изготовлении покупателю говорим один раз, на оформлении заказа (CheckoutPage).
// Все поверхности (карточка, страница товара, липкий бар, корзина) прячут
// блок статуса при пустом label.
const STATUS_LABELS = {
...
export function getAvailabilityTiming(state, leadDays) {
  ...
  switch (state) {
    ...
    // Сроки индивидуального изготовления не анонсируем в витрине — их
    // согласовываем после подтверждения заказа (сообщение на checkout).
    case 'made-to-order':
      return '';
    case 'out-of-stock':
      return '';
