...
//
// Ключ выбора (то, что уходит на бэкенд в selected_model_keys) — `key` детали,
// как его отдаёт GET /api/products. Это непрозрачный идентификатор
// (core/productParts.partPublicId), а НЕ ссылка: магазин продаёт печать, поэтому
// рабочих url на STL/3MF в публичном API больше нет.
//
// Фолбэк на `url` оставлен ради корзин в localStorage, сохранённых до этого
// изменения: там лежат старые url-ключи. Бэкенд (resolvePartSelection) принимает и
// то, и другое, поэтому такая корзина оформляется как раньше, а не падает в 400.
