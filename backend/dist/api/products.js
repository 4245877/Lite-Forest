...
export async function productsRoutes(app) {
    ...
    app.get('/api/v1/products', {
        ...
        const knex = getKnex();
        const { limit = 20, cursor, category_id, include_descendants = false, brand, q, min_price, max_price, sort = 'newest', attr } = req.query;
        const base = knex('products').select('id', 'sku', 'title', 'short_description', 'description', 'price_cents', 'currency', 'category_id', 'brand', 'attributes', 'created_at', 'updated_at');
        ...
        if (typeof min_price === 'number')
            base.where('price_cents', '>=', min_price);
        if (typeof max_price === 'number')
            base.where('price_cents', '<=', max_price);
        ...
        if (sort === 'newest') {
            ...
            if (c)
                base.whereRaw('(created_at, id) < (?, ?)', [c.created_at, c.id]);
        }
        else if (sort === 'price_asc') {
            base.orderBy([{ column: 'price_cents', order: 'asc' }, { column: 'id', order: 'asc' }]);
            const c = decodeCursor(cursor);
            if (c)
                base.whereRaw('(price_cents, id) > (?, ?)', [c.price_cents, c.id]);
        }
            ...
            const c = decodeCursor(cursor);
            if (c)
                base.whereRaw('(rank, id) < (?, ?)', [c.rank, c.id]);
        }
        base.limit(limitPlus);
        const rows = await base;
        const hasMore = rows.length === limitPlus;
        const items = rows.slice(0, limitPlus - 1);
        let nextCursor;
        if (hasMore) {
            ...
            if (sort === 'price_asc' || sort === 'price_desc')
                nextCursor = encodeCursor({ price_cents: last.price_cents, id: last.id });
            if (sort === 'relevance')
                nextCursor = encodeCursor({ rank: last.rank, id: last.id });
        }
